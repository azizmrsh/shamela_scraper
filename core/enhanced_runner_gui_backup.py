...
class EnhancedRunnerGUI:
    ...
    def create_extract_tab(self):
        ...
        # إطار معلومات الكتاب مع تصميم محسن
        book_frame = ttk.LabelFrame(extract_frame, text="📚 معلومات الكتاب", padding="15")
        book_frame.pack(fill=tk.X, padx=10, pady=(0, 15))
        
        # إضافة خلفية للإطار
        style = ttk.Style()
        style.configure('BookFrame.TLabelframe', background=self.light_color)
        book_frame.configure(style='BookFrame.TLabelframe')
        
        # أنماط إضافية للإدخالات
        style.configure('Book.TEntry', font=('Arial', 11), padding=5)
        ...
        # أزرار التحكم مع تصميم جديد
        control_frame = ttk.Frame(extract_frame)
        control_frame.pack(pady=20)
        
        # زر الاستخراج
        self.extract_btn = ttk.Button(control_frame, text="🚀 بدء الاستخراج", 
                                     command=self.start_extraction, style='Success.TButton')
        self.extract_btn.pack(side=tk.LEFT, padx=(0, 10), ipadx=10, ipady=5)
        
        # زر الإيقاف
        self.stop_btn = ttk.Button(control_frame, text="⏹️ إيقاف العملية", 
                                  command=self.stop_operation, state=tk.DISABLED, style='Error.TButton')
        self.stop_btn.pack(side=tk.LEFT, padx=(0, 10), ipadx=10, ipady=5)
        
        # زر المسح
        clear_btn = ttk.Button(control_frame, text="🗑️ مسح البيانات", command=self.clear_extract_form,
    ...
    def create_status_bar(self):
        ...
        # إضافة خط فاصل فوق شريط الحالة
        separator = ttk.Separator(self.root, orient='horizontal')
        separator.pack(fill=tk.X, side=tk.BOTTOM, padx=0, pady=(0, 2))
        
        # عرض الحالة الحالية مع أيقونة
        status_frame = ttk.Frame(self.status_bar)
        status_frame.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # أيقونة الحالة
        status_icon = ttk.Label(status_frame, text="✓", foreground=self.success_color, font=('Arial', 12, 'bold'))
        status_icon.pack(side=tk.LEFT, padx=(5, 0))
        
        # نص الحالة
        self.status_text = ttk.Label(status_frame, text="جاهز للعمل", foreground=self.primary_color,
                                    font=('Arial', 9))
        self.status_text.pack(side=tk.LEFT, padx=5)
        
        # معلومات النسخة وحقوق النشر
        version_frame = ttk.Frame(self.status_bar)
        version_frame.pack(side=tk.RIGHT)
        
        self.version_label = ttk.Label(version_frame, text="Enhanced Shamela v2.1", 
                                      foreground=self.dark_color, font=('Arial', 9, 'bold'))
        self.version_label.pack(side=tk.RIGHT, padx=10)
        
        # عرض الوقت الحالي
        time_frame = ttk.Frame(self.status_bar)
        time_frame.pack(side=tk.RIGHT, padx=(0, 20))
        
        # تحديث الوقت تلقائيًا كل ثانية
        self.time_label = ttk.Label(time_frame, foreground=self.dark_color, font=('Arial', 9))
    ...
    def update_time(self):
        ...
        self.time_label.configure(text=f"⏱️ {current_time}")
        # تحديث كل ثانية
    ...
    def upload_single_file(self, file_path):
        ...
        try:
            ...
            command = [
                ...
                "--db-password", self.db_password_var.get() if self.db_password_var.get() else " "  # استخدام مسافة بدلاً من قيمة فارغة
            ]
            ...
            env['PYTHONUTF8'] = '1'
            # إضافة متغير بيئة لمنع طلب كلمة المرور تفاعلياً
            env['DB_PASSWORD_PROVIDED'] = 'true'
    ...
    def test_database_connection(self):
        ...
        try:
            ...
            connection_params = {
                ...
                'user': self.db_user_var.get(),
                'database': self.db_name_var.get(),
                'password': self.db_password_var.get() if self.db_password_var.get() else " "  # استخدام مسافة بدلاً من قيمة فارغة
            }
    ...
    def log_message(self, message, msg_type="info"):
        """
        إضافة رسالة للسجل مع التصنيف والألوان
        
        أنواع الرسائل:
        - info: رسائل معلوماتية (الافتراضي)
        ...
        try:
            ...
            # تحديد نوع الرسالة بناءً على محتواها إذا لم يتم تحديده
            if msg_type == "info":
                if any(s in message for s in ["✅", "نجح", "تم بنجاح", "🎉"]):
                    msg_type = "success"
                elif any(s in message for s in ["⚠️", "تحذير", "🔔"]):
                    msg_type = "warning"
                elif any(s in message for s in ["❌", "خطأ", "فشل"]):
                    msg_type = "error"
                    
            # إضافة أيقونة مناسبة للرسالة إذا لم تكن موجودة
            icon = ""
            if not any(s in message for s in ["✅", "❌", "⚠️", "🔍", "📊", "🚀", "💾", "🔄", "🎉"]):
                if msg_type == "success":
                    icon = "✅ "
            ...
            # تنسيق الرسالة مع الوقت
            timestamp = datetime.now().strftime("%H:%M:%S")
            formatted_message = f"{message}\n"
            
            # إدراج الطابع الزمني بشكل منفصل لتطبيق نمط مختلف
            self.logs_text.insert(tk.END, f"[{timestamp}] ", "timestamp")
            
            # إدراج الرسالة مع التصنيف المناسب
            self.logs_text.insert(tk.END, icon + formatted_message, msg_type)
    ...
    def extract_single_book_from_category(self, book_id):
        ...
        try:
            # إعداد الأمر - نستخدم save-db بعد extract مع معاملات قاعدة البيانات
            # نستخدم استراتيجية مختلفة: استخراج الكتاب أولاً ثم حفظه في قاعدة البيانات
            
            # الخطوة 1: استخراج الكتاب بدون استخدام قاعدة البيانات
            # تجهيز قائمة الأمر الأساسية للاستخراج فقط
            extract_command = [
                sys.executable,
                os.path.join(current_dir, "enhanced_runner.py"),
                "extract",
                str(book_id),
                "--db-password", " "  # استخدام مسافة بدلاً من قيمة فارغة لتجنب طلب كلمة المرور
            ]
            ...
            # تسجيل مخرجات الاستخراج
            if extract_result.stdout:
                self.root.after(0, lambda: self.log_message(f"📖 كتاب {book_id}: {extract_result.stdout.strip()}"))
            
            if extract_result.stderr:
                self.root.after(0, lambda: self.log_message(f"⚠️ كتاب {book_id}: {extract_result.stderr.strip()}"))
                
            # فحص نجاح عملية الاستخراج
            if extract_result.returncode != 0:
                self.root.after(0, lambda: self.log_message(f"❌ فشل استخراج الكتاب {book_id}"))
                return False
                
            # استخراج مسار ملف JSON من مخرجات الأمر
            output_path = None
            for line in extract_result.stdout.splitlines():
                if "تم الحفظ في:" in line:
                    output_path = line.split("تم الحفظ في:")[1].strip()
                    break
            
            if not output_path or not os.path.exists(output_path):
                self.root.after(0, lambda: self.log_message(f"❌ لم يتم العثور على ملف الإخراج للكتاب {book_id}"))
                return False
                
            self.root.after(0, lambda path=output_path: self.log_message(f"💾 تم استخراج الكتاب {book_id} بنجاح"))
            
            # الخطوة 2: حفظ الكتاب في قاعدة البيانات
            self.root.after(0, lambda: self.log_message(f"🗄️ جاري حفظ الكتاب {book_id} في قاعدة البيانات..."))
            
            # إعداد أمر حفظ الكتاب في قاعدة البيانات
            save_command = [
                sys.executable,
                os.path.join(current_dir, "enhanced_runner.py"),
                "save-db",
            ...
            # تشغيل أمر الحفظ في قاعدة البيانات
            ...
            # تسجيل مخرجات الحفظ في قاعدة البيانات
            if save_result.stdout:
                self.root.after(0, lambda: self.log_message(f"�️ كتاب {book_id}: {save_result.stdout.strip()}"))
            
            if save_result.stderr:
                self.root.after(0, lambda: self.log_message(f"⚠️ كتاب {book_id} - خطأ في قاعدة البيانات: {save_result.stderr.strip()}"))
